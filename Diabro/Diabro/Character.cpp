...
void Character::correctRotation(Ogre::Vector3 pos, Zone* zone, int range){
	...
	// up and down
	else if (!(zone->getCollisionGrid()[behind.x + behind.z * zone->getResolution().x * City::gridScalar]) ||
		!(zone->getCollisionGrid()[front.x + front.z * zone->getResolution().x * City::gridScalar])){
		newDirection.z = 0;
	}

	//whiskers
	else if (!(zone->getCollisionGrid()[topLeft.x + topLeft.z * zone->getResolution().x * City::gridScalar])){
	...
	else if (!(zone->getCollisionGrid()[topRight.x + topRight.z * zone->getResolution().x * City::gridScalar])){
		if (newDirection.x < -0.5f){
			newDirection = Ogre::Quaternion(Ogre::Degree(-collisionRotation), Ogre::Vector3::UNIT_Y) * newDirection;
		}
		else if (newDirection.z > 0.5f)
	...
	else if (!(zone->getCollisionGrid()[bottomLeft.x + bottomLeft.z * zone->getResolution().x * City::gridScalar])){
		if (newDirection.x > 0.5f){
			newDirection = Ogre::Quaternion(Ogre::Degree(-collisionRotation), Ogre::Vector3::UNIT_Y) * newDirection;
		}
		else if (newDirection.z < -0.5f)
	...
	else if (!(zone->getCollisionGrid()[bottomRight.x + bottomRight.z * zone->getResolution().x * City::gridScalar])){
		if (newDirection.x > 0.5f){
			newDirection = Ogre::Quaternion(Ogre::Degree(collisionRotation), Ogre::Vector3::UNIT_Y) * newDirection;
		}
		else if (newDirection.z > 0.5f)
